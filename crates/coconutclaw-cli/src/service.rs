...
fn service_names(cfg: &RuntimeConfig, overrides: &CliOverrides) -> Result<ServiceNames> {
    let instance_override = overrides
        .instance
        .clone()
        .or_else(|| env::var("COCONUTCLAW_INSTANCE").ok());
    let instance_dir_override = overrides
        .instance_dir
        .clone()
        .or_else(|| env::var("INSTANCE_DIR").ok().map(PathBuf::from));

    let mut key = String::new();
    if let Some(instance) = instance_override.as_deref() {
        let normalized = sanitize_identifier(instance);
        if !normalized.is_empty() && normalized != "default" {
            key = normalized;
        }
    } else if instance_dir_override.is_some() && !paths_equal(&cfg.instance_dir, &cfg.root_dir) {
        let base = cfg
            .instance_dir
            .file_name()
...
mod tests {
    ...
    fn service_names_uses_env_selected_instance_name() {
        ...
        unsafe { env::set_var("COCONUTCLAW_INSTANCE", "work") };
        let names = service_names(&cfg, &overrides).expect("service names");
        unsafe { env::remove_var("COCONUTCLAW_INSTANCE") };
    ...
    fn service_names_uses_env_selected_instance_dir() {
        ...
        unsafe { env::set_var("INSTANCE_DIR", "/tmp/coconut/work") };
        let names = service_names(&cfg, &overrides).expect("service names");
        unsafe { env::remove_var("INSTANCE_DIR") };
